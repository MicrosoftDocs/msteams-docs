...
After the app is published, you'd want to measure how your app is performing in the real-world. You'd also be interested in monitoring many data points, such as:

- Who is interested in your app?
- Which users and organizations are using your app?
- How are users engaging with your app?
- Which users churn away after using your app for some time?

This information helps you analyze the data against your business goals and take corrective action by fixing issues. It also helps you to intervene in the user journey or make informed decisions around feature investments, enhancements, and adoption for the app.

**But Teams app usage report must suffice, isn’t it?**

You can track your app’s usage in the [Teams app usage report](../deploy-and-publish/appsource/post-publish/overview.md#analyze-app-usage) in Partner Center within a week after publishing your app on the Microsoft Teams Store. Custom app developers can find [usage analytics for their custom apps](../build-and-test/analyze-your-apps-usage-in-developer-portal.md) in the Developer Portal for Teams. Partner Center's usage report provides standard metrics that enable you to track user demand, user churn, and frequency of usage for your app. These reports are available at an aggregate level, such as:
...
Your app on Teams is a web-based service hosted elsewhere, for example, Azure cloud. It's embedded and surfaced inside Microsoft Teams where users can use your app. This surfacing applies to your app irrespective of the [platform capabilities](../../overview-explore.md) you use, such as message extensions, tabs, bots, meeting extensions, cards, or dialogs (referred as task modules in TeamsJS v1.x). All these capabilities are a means to surface web-based experiences inside Teams.
...
Plan analytics for your Teams app at the development design and solution architecture stage. Examine which of the existing data instrumentation and practices for your core SaaS web app would accumulate to hosted canvas constructs. The constrcuts include app capabilities such as tabs, dialogs, meeting apps, and so on, and you'd need to optimize or build from scratch to surface inside Teams. For Teams-specific capabilities, such as [conversational constructs](#conversational-constructs) like bots, message extensions, and so on, you need to plan and implement analytics instrumentation, capture relevant events, and context from the SDK methods from scratch.
...
- User Principal Name or `loginHint` of the current user in the current tenant (usually user’s email address).
...
Conversation or chat-oriented capabilities include bots, message extensions, cards, and dialogs (displaying an Adaptive Card) that are created for Teams users. Conceptually, these experiences are created for and available only to users specifically inside Teams. You need to capture Teams-specific events and handle them for Teams-specific instrumentation in your code for these constructs from scratch. A bot can access additional context data about a team, chat, meeting, 1:1 call, or group call where the user installs it.
...
Since a message extension is based on the bot channel, most of what applies to a bot, applies to a message extension as well.

## Aggregate and user-specific insights

Besides the obvious metrics such as daily, weekly, monthly active users, and time spent in your app, implementing analytics thoughtfully for your Teams app per the guidance mentioned allows you to get insights such as:

- **Aggregate metrics**
  - Which platform capabilities, surface areas (for example, tabs, bots, message extensions) and UI constructs (cards, dialogs, Stage Views) used in your Teams app found the most usage by your users?

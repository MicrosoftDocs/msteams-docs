...
| --- | --- | --- | --- |
| `APP_ID` | string | Yes | For third-party apps use the `APP_ID` from manifest or Teams admin center as the `APP_ID` is identical. For LOB or custom apps, use the `APP_ID` from Teams admin center or read using [Graph API](/graph/api/application-list&tabs=http#examples). The [validDomains array](~/resources/schema/manifest-schema.md#validdomains) in the manifest for `APP_ID` must contain the domain for `url` if `url` is in the URL. The app ID is already known when a task module is invoked from a tab or a bot, which is why it isn't included in `TaskInfo`. |

...
Teams offers multiple surfaces and integration points that developers can build custom experiences on top of. These surfaces and integration points are called *app capabilities*, and an implementation that builds on top of one or more of them is a *Teams app*.

Developers use the *bot* app capability to create bots and agents: conversational assistants that participate in Teams chat. The terms "bot" and "agent" are often used interchangeably, but in the modern AI landscape, agents are distinguished from bots by use of large language models (LLM). Agents use LLMs to understand and generate natural language, adapt to context, and dynamically perform actions on behalf of users.
...
Creating and hosting a runtime is not enough to make an app available in Teams. Teams apps require registration and configuration across a few different surfaces:

- An app's *manifest* is a JSON configuration file that represents the app on the Teams platform
- Most Teams apps have an *app registration* identity in Microsoft Entra ID to enable authenticated for access to data and services
- An agent's runtime interacts with the Teams platform via the Bot Connector service, which requires a *Bot Service resource* configuration in Microsoft Azure
...
An app's manifest contains all the configuration needed by the Teams platform to distribute the app and present it in Teams. For example, it includes the app's name and description, the unique ID of its Entra ID app registration, and a list of Teams operations the app needs permissions to access. It also configures each app capability the app implements - for agents, this includes the ID of its Bot Service resource.
...
Microsoft Entra ID is the identity and access management service used by Teams and Microsoft 365. Most Teams apps have an *app registration* in Entra ID that globally identifies it and allows it to participate in identity and authorization flows within the Microsoft 365 ecosystem.

An app's registration enables administrators to grant it access to organizational data and services. It also provides the infrastructure needed for the app to perform enterprise single sign-on and OAuth authentication, allowing it to access resources on behalf of consenting users. Many agents use application and user permissions to access the Microsoft Graph API to access the workplace data that powers their collaboration features. TODO almost done with this sentence; clean it

In the quickstart, `teams app create` creates an Entra ID app registration for your agent in your Entra tenant.

### Bot Services: the Teams interface for agents

The Bot Connector service is the interface between the Teams platform and an agent's runtime. It is what calls an agent's runtime when events are triggered in Teams, and what the agent runtime calls for most Teams-related operations. Simplifying and structuring interactions with the Bot Connector service is the main purpose of Teams SDK.

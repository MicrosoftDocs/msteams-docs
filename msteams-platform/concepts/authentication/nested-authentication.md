...
> [!NOTE] > Token prefetching is currently supported only in the Microsoft Teams web and desktop clients.
...
- **Have a fallback**: The NAA flows offer compatibility across the Microsoft ecosystem. However, your app might appear in down-level or legacy clients that aren't updated to support NAA. In such cases, your app can't support seamless SSO, and you might need to invoke special APIs for interacting with the user to open authentication dialogs. For more information, see [enable SSO for tab app](../../tabs/how-to/authentication/tab-sso-overview.md).

  > [!NOTE] > You mustn't use NAA if you're using a non-Microsoft Entra identity provider, you can use pop-up authentication instead.
...
* **Test your app in multiple environments**: If your app needs to function in both web view and browser deployments, test your app in both environments to ensure it behaves as expected. Some APIs that work in the browser might not function within web views.

## Code sample

| Sample name | Description | .NET | Node.js |
|:------------|:------------|:-----|:--------|
| Nested app authentication | This sample demonstrates Microsoft Entra single sign-on (SSO) within a Microsoft Teams tab, using the On-Behalf-Of (OBO) flow to call Microsoft Graph API on behalf of the user. | [View](https://github.com/OfficeDev/Microsoft-Teams-Samples/tree/main/samples/tab-nested-auth/csharp) | [View](https://github.com/OfficeDev/Microsoft-Teams-Samples/tree/main/samples/tab-nested-auth/nodejs) |

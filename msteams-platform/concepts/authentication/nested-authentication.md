...
> [!important]
>
> * The agent or app's Microsoft Entra ID should be used as the client ID for each NAA prefetch request entry in the `clientId` field within the `nestedAppAuthInfo` array. This is an optional field that is used to prefetch the token for that particular entry. If any entry doesn't have the `clientId` in the `nestedAppAuthInfo` array, then `webapplicationinfo.id` will be used as the clientId for that entry's NAA prefetch request.

...
1. Link NAA user profile with bot auth profile: This process ensures that the user's session in the tab is securely authenticated using the token generated by the backend, enabling seamless access to protected resources without requiring repeated logins.
    ...
    1. The app needs to store this access token in a cookie associated with the tab authentication flow.
...
f. The app now needs to set an actual access token generated in its backend for the NAA based authentication and set it in the tab auth cookie as part of the auth redirect.
g. Post authentication success the account linking page needs to now call its backend to link the NAA based profile with the Bot auth Profile.

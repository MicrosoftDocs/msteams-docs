...
business rule changes and adhere to the [Microsoft commercial marketplace certification policies](https://docs.microsoft.com/en-us/legal/marketplace/certification-policies).
...
We've cross-referenced the guidelines to the Microsoft commercial certification policies and added Do’s and Don’ts with examples from pass/fail scenarios encountered in our validation process.

Certain guidelines are marked as **Mandatory Fix**. If your app submission doesn't meet these guidelines, you'll receive a failure report from us with steps to mitigate. Your app submission will pass Microsoft Teams Store Validation only after you have fix the issues. Other guidelines are marked as **Suggested Fix**. 

For an ideal user experience, we suggest you fix these, however, your app submission won't be blocked from publishing on the Teams store, if you choose not to fix. 
...
An app's name plays a critical role in how users discover it in the store. Remember the following:
...
* Must be unique. If your app is listed in the Microsoft Teams store and  Microsoft AppSource and you want to list another app specific to a geography,your submission must meet the following criteria:
...
Your app must enable group collaboration, improve an individual's productivity, or both. Apps intended for team bonding and socializing must be collaborative and designed for multiple participants. These types of apps also shouldn't require a more time investment (>60 mins per session) or perceptively affect productivity.
...
App feature names in buttons and other UI text must not conflict with terminology reserved for Teams and other Microsoft products. For example, **Start meeting**, **Make call**, or **Start chat**. If required, include your app name, such as **Start Contoso meeting** instead of **Start meeting**.
...
When an app authenticates users with an external service, consider the following:
...
* **Sign-in, sign-out, and sign-up experiences**:
  ...
  * Apps that depend on external accounts or services must provide a way forward for new users to sign-up or contact the app publisher to learn more about the services and potentially acquire credentials.  Way forward must be provided in the app’s manifest, AppSource long description, and app first run experience (bot welcome message, tab setup/config page).
  * Apps that require tenant admin to complete one time setup must clearly call out dependency on tenant admin to configure the app (before any other tenant user can install and use the app).
    ...
    Dependency must be called out in the app’s manifest and AppSource long description, all first run experience touchpoints including bot welcome message, tab setup or config page, help text as considered necessary as part of bot response or compose extension or static tab content. 

* **Content sharing experiences**: 
  * Apps that require authentication with an external service to share content in Teams channels must clearly state how to disconnect or unshare content if that feature is supported on the external service the in help documentation (or similar resources). It doesn't mean the ability to unshare content must be present in your Teams app.
...
Apps must not ask users to make payments within the Teams interface (see the following image). Transmit financial instrument details to users through a bot interface.
...
* You can determine whether an account is active indefinitely or for a limited time. If the account expires the app shouldn't show UI, text, or links indicating the need to pay.
...
* Your manifest must conform to a publicly released manifest schema. For information, see the [manifest reference](~/resources/schema/manifest-schema.md). Don't submit your app using a preview version of the manifest.
* If your app includes a bot or messaging extension, the app manifest must be consistent with Bot Framework metadata like bot name, logo, privacy policy link, and terms of service link.
...
* The outline version of your icon displays when your app is in use and **hoisted** on the app bar on the left side of Teams and when a user pins your app's messaging extension. The outline must be 32x32 pixels and can be white with a transparent background or transparent with a white background. The icon shouldn't have any extra padding around the symbol.
...
You must have a short and long description of your app. Ensure App configurations and Partner Center description are the same.
...
* Make unverified claims (for example: best, top, ranked), unless it comes with the source of the claim.
...
* Shouldn't contain URLs that are broken or for beta or staging purposes.
...
Support URLs must include your contact details or other steps for users to raise a support ticket. If your support URL is hosted on GitHub, the GitHub page must be under your ownership and include your contact details.
...
* The secondary and tertiary pages in a tab must be opened in a level 2 and level 3 view in the main tab area, which is navigated via breadcrumbs or left nav. You can also include the following components to aid tab navigation:
    * Back buttons
    * Page headers
    * Hamburger menus
* Tab shouldn't have horizontal scroll. Whiteboarding apps and other apps that require a larger canvas to allow users to collaborate without a perceived broken app experience, can use horizontal scroll depending on their business need.  
...
* Deep links in tabs must not link to an external webpage but somewhere within Teams. For example, task modules or other tabs.
  ...
  > ![View Button not Linked](/assets/images/submission/validation-navigation-v(iew-button-not-linked-static-tab.png)
...
* Tabs must provide value beyond just hosting an existing website.
  ...
  >![Usability App Provides Workflows](/images/submission/validation-usability-app-provides-workflows.png)  
  > App provides workflows in a configurable tab that are valuable to the channel members within a Team. Developer has scoped relevant workflows for implementation within Teams.

* Users can undo their last action in the tab.
  >![Usability Website I-Frame](/images/submission/validation-usability-website-i-framed.png)  
  > Entire website I-framed within configurable tab. Website has header, footer and native navigational elements that navigate user from the home page without a way back.

  >![Usability Teams app identical](/images/submission/validation-usability-teams-app-identical-website.png)  
  >  Teams app identical to the website. All workflows can be completed within the website, unclear value proposition within Teams. Website pinned to the channel using “+” offers same functionality as Teams app.

* Content must not truncate or overlap within the tab.
  >![Usability Content Truncation](/images/submission/validation-usability-content-truncation.png)  
  > Content truncation within the tab.
...
* Tabs must be responsive to Teams themes. When a user changes the theme, the app's theme must reflect the selection.
  ...
  >![Usability Resposive tabs](/images/submission/validation-usability-responsive-tabs.png)  
  > Responsive to “Dark” theme within a Teams meeting.

  >![Usability unresponsive tabs](/images/submission/validation-usability-unresponsive-tabs.png)  
  > Unresponsive to **Dark** theme within a Teams meeting.
...
* Tabs must use Teams-styled components such as, Teams fonts, type ramps, color palettes, grid system, motion, tone of voice, and so on, whenever possible. Follow tab design guidelines.
  ...
  >![Usability different font](/images/submission/validation-usability-app-uses-diff-font.png)  
...
* Tab content in the iframe must not include features that mimic Teams core capabilities. For example, bots, messaging extensions, calling, meeting, etc..
  >![Usability Chatbot within tab](/images/submission/validation-usability-chatbot-withing-tab.png)  
  > Includes a chatbot within the tab.

* Content in the landing page of the configurable tabs must be contextually same for all members of the channel. Configurable tabs help conversations about common content in a central location such as a Teams channel. Ensure you use these collaborative tabs only to display content that is same for all users. 

* Content in the landing page of configurable tabs must not be scoped for individual use nor include personal content such as **My Tasks** or **My Dashboard**. If your app requires provision of a personal scope view for the user to enhance efficiency or workplace productivity, use filtered views, deep links to personal apps or navigate to L2/L3 views within the configurable tab and keep the landing page contextually the same for all users.

  >![Usability Configurable tab personal Info](/images/submission/validation-usability-content-truncation.png)  
  > Configurable Tab landing page has personal content such as “My day” and “To-do” that are specific to a particular user and irrelevant to the other members of the channel.

* Configurable tabs must have focused functionality. 

  >![Usability Configurable Nested tabs](/images/submission/validation-usability-configurable-nested-tabs.png)  
  > Configurable tab has several nested tabs.
...
> ![NOTE]
> The following section references Microsoft Commercial Marketplace Policy # 1140.4.3.
...
* Listing supported bot commands in your app configurations is highly recommended. These commands display in the compose box when a user tries to message your bot.
  ...
  > ![Bot Commands Listed](/images/submission/validation-bot-commands-listed.png)

  > Bot commands listed in manifest.

  > ![Bot Commands not Listed](/images/submission/validation-bot-commands-not-listed.png)

  > Bot commands not listed in manifest.

* All commands that your bot supports must work correctly, including generic commands such as **Hi**, **Hello**, and **Help** command.

 >![Bot Help Command](/images/submission/validation-bot-help-command.png)  
...
* Bot commands shouldn't show a dead end to a user, the commands must always provide a way forward.
  ...
  > ![Bot Command Dead End])(/images/submission/validation-bot-commands-dead end.png)  
  > Bot command with a dead end to the user.
...
* Notification-only bots must send a welcome message that conveys it won't reply to users' messages.
  ...
  > Notification only bot sends it will not reply to user messages.

  > ![Notification Bot doesn't convey](/images/submission/validation-bot-notification-bot-doesn't-convey-message.png)  
  > Notification only bot doesn't send a welcome message.

> [!TIP]
> In welcome messages to individual users, a carousel tour can provide an effective overview of your bot and any other app features to encourageusers try bot commands. For example, **Create a task**.
...
* **Bot messages in personal apps**: Don't send multiple messages in quick succession. Send one message with complete information. Avoid multi-turn conversations to complete a single workflow. Use a form (or task module) to collect all inputs from a user at one time.
  ...
  > Bot using a task module to collect information.

  >![Bot Using Mutliple Conversation](/assets/images/submission/validation-bot-messages-using-mutliple-conversation.png)  
  > Bot using multi turn conversation to complete a repetitive workflow.
...
Bot experience must be responsive on mobile.
Bot responses must provide a way forward where applicable. Bots must be responsive and display a smooth error message for failures.
Bot should provide contextual information (including the message’s origin) for personal messages that are sent to users based on triggers in a collaborative scope.
...
1.	Posted card/text provides adequate details requiring no further user action.
1.	Posted card/text provides adequate preview information for a user to take action or decide to view further details in a link opening outside Teams.
1.	Posted card provides preview information and basic inline user actions (For example: Assign to me) and user shouldn't navigate outside Teams for all actions (regardless of complexity).
1.	Posted card provides preview information and enables users to view further details or take all actions within Teams.

For a notification only bot to pass app validation, points 1 and 2 are required as baseline to ensure user experience isn't broken. Apps that provide only notifications with content such as **You have a new notification, click to view**, and require user to navigate outside of Teams don't provide significant value within Teams.

  >![Bot inadequete information](/assets/images/submission/validation-bot-notification-only-inadequete-info.png)  
  > Inadequate information in notification preview.

  >![Bot Adequete information](/assets/images/submission/validation-bot-notification-only-adequete-info.png)  
  > Adequate information in notification preview to minimize context switching.

### Use of team roster with no extra functionality within the team

An app, which can only read a team roaster to store user IDs and send proactive notifications to the users and doesn't provide any other functionality has no end-user value in installing the app. It's just a hidden app reading team roster.

Use bots to obtain teams roster only in the following scenarios (applicable only to apps with channel bot or group chat bot with no bot functionality in these scopes, app offers no other capability in channel/group chat):

  * App requires targeting. For example: Send different proactive messages to users from team A vs users from team B.

    >![Team Roaster for Trageting](/assets/images/submission/validation-team-roaster-target-users.png)  
    > App uses Teams roster for randomly matching members for coffee chats.

  * App requires team event triggers. For example: Check for changes in team roster to send/hold back notifications.

    >![Team Roaster Target users](/images/submission/validation-team-roaster-target-users.png)  
    > App uses Teams roster for randomly matching members for coffee chats.

  Such apps must provide appropriate welcome messages with the personal scope and context in 1:1 notification (what team/channel usage triggered the notification).
	Avoid the use of team roaster in scenarios where app needs to send 1:1 notification based on event triggers (for example: visitor management notifications) or mass notifications that are sent to entire organization. Use app setup and app permission policies.
   > [Teams Roaster External Event Triggers](/assets/images/submission/validation-team-roaster-external-event-triggers.png)  
   > App uses Teams roster for sending 1:1 notification based on external event triggers.
 
## Messaging extensions

> [!NOTE]
> The following section references Microsoft Commercial Marketplace Policy # 1140.4.4.
...
* Allow users to trigger actions on a message without completing intermediate steps, such as signing in.
  ...
  > No intermediate step available.

  >![Intermediate Step Available](/assets/images/submission/validation-messaging-extension-intermediate-step-available.png)  
  > Intermediate step **Sign in** available.

* Pass the message context to the next work state.

  >![App Passes Message]()  
  > App passes message context (**Hello Good Morning Meeting**) to next work state (task module).

  >![App doesn't Pass Message]()  
  > App does not pass message context (**Hello Good Morning Meeting**) to next work state (poll).
...
for **Start Meeting**, **Upload file to DocuSign** for **Upload file**).
  ...
  > ![Action Command Host name](/images/submission/validation-messaging-extension-action-command-host-name.png)    
  > Action command incorporates host app name.

  > ![Action Command verb](/images/submission/validation-messaging-extension-action-command-verb.png)  
  > Action command uses a generic verb (**New Task**).
...
Messaging extensions must preview recognized links in the Teams compose box. Don't add domains that are outside your control (either absolute URLs or wildcards). For example, `yourapp.onmicrosoft.com` is valid but `*.onmicrosoft.com` isn't valid. Top-level domains also are prohibited (for example, `*.com` or `*.org`).
  ...
  ![Previw link Unfurls](/images/submission/validation-preview-links-unfurls-link.png)  
  ...
  >![Preview link doesn't unfurl](/images/submission/validation-preview-links-does-not-unfurl-link.png)  
  > Message extension does not unfurl link in the Teams compose box.
...
* Search-based messaging extensions must provide text that helps users effectively search.
  ...
  > Text to search effectively available.

  >![Help Tetxt not Available](/assets/images/submission/validation-search-commands-text-not-available.png)  
  > No text prompt available to search effectively.

* @mention executables must be clear, easy to understand, and readable.
  >![Search Command Unclear Excecutable](/images/submission/validation-search-command-unclear-executable.png)  
  > Unclear @mention executable.

### Search based messaging extension only apps

Apps that consist of search-based messaging extension only provide user value by sharing cards that allow for contextual conversations without context switching.
...
1.	Posted card provides preview information and basic inline user actions (for example: Approve) and user 
isn't required to navigate outside Teams (regardless of complexity).
1.	Posted card provides preview information and enables users to view further details or take all 
...
baseline to ensure user experience isn't broken.

  >![Search based messaging inadequate](/images/submission/validation-search-based-messaging-ext-inadequete-info.png)  
  > Inadequate information in posted card.

  >![Search based messaging adequete](/images/submission/validation-search-based-messaging-ext-inadequete-info.png)  
  > Adequate information in posted card.

Apps with an action-based messaging extension only should have the end state as:

  *	App posts relevant action as a notification either in the context where message extension was invoked or in 1:1 bot chat depending on user scenario.
  *	App allows users to share cards with other users based on the action taken.
	
Apps must not take silent action (for example:Create a ticket based on a message in a channel and not send a notification or provide a way for the user to share ticket details after the ticket is created).

Apps with Link Unfurling as the only feature don't provide significant value within Teams. Consider building more workflows in your app.

## Task modules

> ![NOTE]
> The following section references Microsoft Commercial Marketplace Policy #1140.4.2.
...
Refer task module design guidelines.

  >![Task module Displays Components](/images/submission/validation-task-module-displays-components.png)  
  > Task module displays components relevant to the specific action (creation of poll in this example)

  >![Task Module Embeds app](/images/submission/validation-task-module-embeds-app.png)  
  > Task module embeds the entire app.

Consider using a stage view to display rich content to the users such as a page and a dashboard to render your content in a full screen sized canvas.
...
>[!NOTE]
> The follwoing section references Microsoft Commercial Marketplace Policy #1140.4.2.
...
Meeting extensibility apps must offer a responsive in-meeting experience aligned to the Teams meeting experience. Pre and post-meeting experiences arn't mandatory.

Pre-meeting app experience: Users can find and add meeting apps, perform pre-meeting tasks such as developing a poll to survey the meeting participants. A pre-meeting experience must be relevant to the workflow of the meeting.

Post-meeting app experience: Users can view the results of the meeting such as poll survey results or feedback and other app content. A post-meeting experience must be relevant to the workflow of the meeting.
...
* Must have a back button if there's more than one layer of navigation.
  ...
  >![In-meeting Back Button Available](/images/submission/validation-in-meeting-exp-back-button.png)  
  > Back button present.

  >![In-meeting Back Button Absent](/images/submission/validation-in-meeting-exp-back-button-absent.png)  
  > Back button absent.
...
* Must align with the center of the meeting stage.
  ...
  >![In-meeting dialog not aligned](/images/submission/validation-in-meeting-dialog-not-aligned.png)  
  > In meeting dialog not aligned to center of the meeting stage.

* Should be dismissed once a user selects a button or performs an action.

### Together mode

The following are the best practices for a scene building experience: 
...
* Maximum scene size is 10 MB.
* Maximum size of each image is 5 MB. A scene is a collection of multiple images. 
...
* All the notification triggers specified in your app configuration should get a notification in the app.
...
>[!NOTE]
> This section reference Microsoft Commercial Marketplace Policy # 1140.6.
...
* Publisher Verification: Helps admins and end users understand the authenticity of app developers integrating with the Microsoft identity platform. When completed, a blue **verified** badge displays on the Azure Active Directory (Azure AD) consent dialog and other screens. For more information, see frequently asked questions, how to mark your app as publisher verified, and troubleshoot publisher verification.
   >![Publisher verification blue badge]()  
   > Blue verified badge on AAD consent dialog.

* Publisher Attestation: A process in which you share general, data handling, security and compliance information to help potential customers make informed decisions about using your app.
>[!NOTE]
...
>[!NOTE]
> This section reference Microsoft Commercial Marketplace Policy # 1140.7
Apps must not display advertising, including dynamic ads, banner ads, and ads in message

## Significant Value

>[!NOTE]
> This section references Microsoft Commercial Certification Policy #100.8. 

Frequently failed case examples are highlighted below.

>[!NOTE]
> This is not an exhaustive list. This is intended to inform you about frequently observed failures during the app validation process, where the app is deemed to NOT offer significant value within Teams. This will let you make the right product decisions for your app at an early stage.

 :::image type="icon" source="</images/submission/validation-incorrect-icon.png>"::: App doesn’t provide any value worth the user effort to install and learn the available app workflows.
 
 :::image type="icon" source="</images/submission/validation-correct-icon.png>"::: Apps must offer a usable software solution to provide significant benefits such as enhanced efficiency or improved workplace collaboration to justify the user investment in learning to use the app. Consider redesigning your app to include more relevant workflows/scenarios.
  
  All core workflows in the app redirect outside Teams.
  
  Apps with all core workflows redirecting outside Teams adversely affect user productivity with constant and frequent need for context switching. Consider bringing the core workflows envisioned for your app within Teams. 
  App is too simple and doesn't offer any user benefit such as improved productivity, collaboration, or enhanced efficiency.
 
 Simple utilities and apps with limited scope aren't a good fit for the marketplace. Apps must offer a usable software solution that provides significant benefits such as enhanced efficiency or improved workplace collaboration. Consider adding more workflows/scenarios in your app.
 App doesn't provide any value beyond the already existing core functionalities offered by Teams nor does it offer any specific interoperability.
  
  Unless your app offers specific interoperability with Teams, consider adding more functionalities relevant to your app, beyond the core functionalities offered by Teams that your app mimics.
 
 Link unfurling only app.
 
 Apps with limited scope aren't a good fit for the marketplace. Apps should provide significant benefits such as enhanced efficiency, improved workplace collaboration, innovative features, or strategic advantages.
 
 Together mode scene only app.
  
  Apps with limited scope aren't a good fit for the marketplace. Apps should provide significant benefits such as enhanced efficiency, improved workplace collaboration, innovative features, or strategic advantages.
  App used as a marketing vehicle provide users with a trial period or demo and asks for premium or upgrade to get user customized product or app after trial period ends. 
 
 Teams apps must provide a usable software solution. Apps that are used as a marketing vehicle don’t add value to the Teams marketplace. Consider building usable workflows in your app to provide enough value for the user to justify the investment it takes to learn and use them. 
  
  Notification only bots that provide zero or limited details or background for the notification and requires a user to go to a website for getting relevant information.
  
  Apps with notification only bots that provide insufficient details for the notification and requires a user to go to a website for getting relevant information adversely affect user productivity with frequent need for context switching. Notifications must provide enough user value by including relevant information to minimize the need for context switching. Consider bringing in relevant information within Teams using task modules.
  
  Only capability of the Teams app is a bot with all actions redirecting outside of Teams.
  
  Consider bringing your app’s core workflows within Teams, using adaptive cards and task modules to surface information and capture user input, in your bot’s response. 
  
  Search based message extension only Teams app that provides limited details or background in the shared card and requires a user to go to a website for getting relevant information.
  
  Search based messaging extension only apps that provide insufficient details for the notification and require a user to go to a website for getting relevant information adversely affect user productivity with frequent need for context switching. Such apps must provide enough user value by including relevant information in the shared card to minimize the need for context switching.
  
  Action based messaging extension only app that completes the action without a feedback loop.
  
  Standalone actions should always have a logical way forward for users. Apps should make users aware of the status of their action while completing a workflow. Consider adding a feedback loop in your action based messaging extension only Teams app. 
  
  Teams app acts as a container containing another app that is the main app.
  
  Core workflows mentioned in the app manifest description must be offered within the app downloaded from the marketplace without the need for downloading and installing a separate app to access and complete the core workflows. Consider redesigning your app to comply with commercial marketplace certification policies.

#### Mobile Meeting Extensibility Guidelines 

**Phase 1 – Enable Apps for Meetings**

Manifest Must Haves:

  * **groupchat** scope in the configurable tabs
	
  * Context to have **meetingChatTab** or **meetingDetailsTab** and **meetingSidePanel**
	
Sample JSON here in code snippet: Enable and configure your apps for Teams meetings - Teams | Microsoft Docs

**Phase 2 – Mobile App Best Practices (Optional but Recommended)**

 *	Use in-meeting dialogs to display lightweight information.
	
 * Not recommended for complex interactions because of the size.

*	Avoid displaying data grids, tables, and other complex information. 

*	Any few more. They are optional but ensure good quality apps are created by Partners. 
	
Details here: Microsoft Teams Meeting Extensions Design Guidelines [LIVE] – Figma

**Phase 3 – Viewing Apps on Mobile meetings**

Apps don’t just appear on Teams Mobile for Meetings. They need to be WHITELISTED by store validation team!
Otherwise, the apps won't show up on Mobile

Key checklist for mobile:
 * Content is fully responsive to small screen size
*	No paywalls or subscription related information
*	No redirects to payment related pages
	
    **Phase3, Step1 – Pre/Post Meeting, App Validation**

*	This ensures the app in the pre/post meeting follows the Mobile guidelines
*	Maintained by Mobile Platform team (Ankur Yadav, PM)

    **Phase3, Step2 – During Meeting, App Validation**

*	This ensures the app experience during meeting follows the Mobile guidelines
*	Maintained by Mobile Platform team (Supriti Bhan, PM) after ensuring Pre/Post Validation is passed
  > ![NOTE]
  > Content Bubble and Banner don't need whitelisting.
	
Complete the steps listed earlier to Whitelist the app. If the app isn't Whitelisted, fix the relevant issues. 

Sideloaded and LOB apps don’t need whitelisting and validation.

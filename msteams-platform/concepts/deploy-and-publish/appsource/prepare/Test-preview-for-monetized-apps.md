...
Testing your app before publishing is crucial. It helps you understand user issues, rectify them, and improve the overall user experience. Let's delve into the detailed steps to create a test or preview offer, configure your app with the preview offer ID, and understand error scenarios with their respective fixes.
...
To create a preview offer ID, use the **AppSource preview** link in Microsoft Partner Center. Ensure that the SaaS offer is in the **Preview creation** phase. Follow these steps to generate the preview offer ID:
...
3. Choose the SaaS offer you want to preview.
4. Add [preview audience](/azure/marketplace/create-new-saas-offer-preview) for your SaaS offer.
5. Select the **AppSource preview** link under **Go Live** to find the preview offer ID in the browser address bar with the format *publisherId.offerId-preview*.
...
6. Copy the preview offer ID from the browser address bar.
    ...
    If the preview offer is limited to non-US regions, the Developer Portal for Teams might not be able to find your offer. You can either add the United States to the list of [markets, pricing, and availability](/azure/marketplace/create-new-saas-offer-plans) or ignore the error and select **Preview in Teams** to upload a custom app to Microsoft Teams.
...
Before you begin, sign in to the **Developer Portal** with a developer account that has **Preview audience**. This allows users to view your subscription plans in the Microsoft Teams Store.

After generating the preview offer ID, link it to your Teams app. To link the offer ID, follow these steps:
...
3. Choose the app to link the SaaS offer.
4. Under **Advanced**, select **Plans and pricing**.
5. Enter the **Publisher ID** and **Offer ID**. Ensure the offer ID has *-preview* as a suffix.
...
After configuring your app with the preview offer ID, create an updated app package and upload it to Teams to test the end-to-end purchase experience. You can also select **Preview in Teams** in the Developer Portal for Teams to launch your app quickly in the Teams client.
...
2. If `offerId` specified in the app manifest isn't a preview offer, the app shows the following warning message to the user and custom app upload is disabled:

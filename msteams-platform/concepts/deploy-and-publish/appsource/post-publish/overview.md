...
* To remove your app from Microsoft AppSource, in your [Microsoft Partner Center](https://partner.microsoft.com/dashboard/v2/membership/workspace/overview) account, go the **Product Overview** page and select **Stop Selling**. This action removes the app from all marketplace listings and user interfaces. The app is no longer discoverable, installable, or available for acquisition. Any content previously posted by the app, such as messages from a bot are visible in user history.

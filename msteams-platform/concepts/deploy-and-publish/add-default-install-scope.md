...
To enhance Teams app experience, you can block an app from users by default until admin allows the app. For example, Contoso Electronics has created a help desk app for Teams. To enable appropriate functioning of the app, Contoso Electronics wants the customers to first configure specific properties of the app. The app is blocked by default and is available to users only after an admin allows it.
...
> [!NOTE]
> For custom apps, `defaultBlockUntilAdminAction` is not supported. If you upload a custom app with this property for use in your personal or team scope, it is not blocked. If you submit any custom app for admin approval, it is blocked till an admin approves it irrespective of the value of `defaultBlockUntilAdminAction` parameter.

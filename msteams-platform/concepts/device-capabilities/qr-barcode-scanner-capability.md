...
You can use [Microsoft Teams JavaScript client library](/javascript/api/overview/msteams-client?view=msteams-client-js-latest&preserve-view=true), which provides the tools necessary for your app to access the user’s [native device capabilities](native-device-permissions.md). Use the [scanBarCode](/javascript/api/%40microsoft/teams-js/microsoftteams.media?view=msteams-client-js&) API to integrate the scanner capability within your app.

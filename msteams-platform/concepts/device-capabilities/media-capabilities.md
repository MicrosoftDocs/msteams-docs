...
| API      | Description   |
| --- | --- |
| [**captureImage**](/javascript/api/%40microsoft/teams-js/media?view=msteams-client-js-latest) (**Camera)**| This API allows users **capture or select media from the gallery** and share as a file. The API allows users to only capture or select single image. In response to `captureImage`, the captured or selected image is shared as a file. |

...
#### Previewing the Adaptive card

The preview property lets developers customize what the search results of a messaging extension will look like. Currently this feature only supports thumbnail or hero cards, but you can make it display your adaptive card by simply setting its preview property to hero or thumbnail. If no preview property is specified, the preview will fail and nothing will be displayed. The appropriate payload to do this is shown in the next example.

#### Example payload with correct values for preview

...
Microsoft Entra ID is the identity and access management service used by Teams and Microsoft 365. An app registration in your developer Entra tenant is a globally unique identity that enables an agent to authenticate to secured resources and participate in user delegation flows (single sign-on and OAuth) within the Microsoft ecosystem.
...
- The endpoint URL of the agent's runtime. This URL will receive activity events with information about user activity in Teams.
- A reference to an Entra ID identity used for agent runtime authentication. The agent's runtime must be configured with credentials for this identity to authenticate to Bot Connector. In most cases, this identity is an Entra ID app registration.
...
1. A standalone Bot Connector registration
1. A Teams Platform registration with a starter app manifest. Both this registration and the standalone Bot Connector registration are associated with the account used to sign in to the CLI.
...
- Try the [quickstart](../agents-in-teams/quickstart-create-agent-teams-sdk.md), where you'll create and register an agent using `teams app create`.

...
| [Agent connector](../m365-apps/agent-connectors.md) | An app manifest property (`agentConnectors`) that declares an external system, such as a remote Model Context Protocol (MCP) server, that Microsoft 365 agents can use to discover and call tools. An agent connector isn't the same as a [Copilot connector](#c), which indexes or federates data for Copilot and Microsoft Search. <br> **See also**: [App manifest](#a); [MCP](#m); [Plugin](#p) |
...
| [App manifest](/microsoft-365/extensibility/schema/) | The app manifest (previously called Teams app manifest) describes how the app integrates into Microsoft Teams and also how to extend your app to run across Microsoft 365. Your manifest must conform to the [app manifest schema](https://developer.microsoft.com/json-schemas/teams/v1.11/MicrosoftTeams.schema.json). |
...
| App tray | An application tray located on the bottom bar of a Teams mobile app. It collects all apps that are open but not in use or active. <br>**See also**: [Teams Mobile](#t) |
...
| [Authorization](/microsoft-365/extensibility/schema/root-authorization)| An app manifest property (`authorization`) that specifies and consolidates authorization related information for the app. <br> **See also**: [Permission](#p)|
...
| [Configurable tab](/microsoft-365/extensibility/schema/root-configurable-tabs)| Configurable tabs are also known as channel or group tabs. Configurable tabs are used when your app experience has a team channel tab experience, which requires extra configuration before it's added. <br> **See also**: [App manifest](#a)|
| [Configuration URL](/microsoft-365/extensibility/schema/root-configurable-tabs#configurationurl)| An app manifest property (`configurationUrl`) where the HTTPS URL to use when configuring the tab or connector. <br> **See also**: [App manifest](#a)|
...
| [Compose Extensions](/microsoft-365/extensibility/schema/root-compose-extensions) | An app manifest property (`composeExtensions`) that refers to message extension capability. It's used when your extension needs to either authenticate or configure to continue. <br>**See also**: [App manifest](#a); [Message extension](#m) |
| [Command Box](/microsoft-365/extensibility/schema/root) | A type of context in app manifest (`commandBox`) that you can configure to invoke a message extension from Teams command box. <br>**See also**: [Message extension](#m)  |
| [Command lists](/microsoft-365/extensibility/schema/root-bots-command-lists)| An app manifest property (`commandLists`) that consists of a list of commands that the bot supplies, including their usage, description, and the scope for which the commands are valid. For each scope, you must use a specific command list. <br> **See also**: [App manifest](#a)|
...
| [Content URL](/microsoft-365/extensibility/schema/root#statictabs)| An app manifest property (`contentUrl`) where the HTTPS URL points to the entity UI to be displayed in the Teams canvas. <br> **See also**: [App manifest](#a)|
...
| [Copilot connector](/microsoft-365/copilot/connectors/overview) | A Microsoft 365 Copilot connector indexes or federates external content into Microsoft Graph so that Microsoft 365 Copilot and Microsoft Search can reason over it, and it's managed under **Copilot** > **Connectors** in the Microsoft 365 admin center. A Copilot connector isn't an [agent connector](#a), which registers a remote MCP server in the app manifest and doesn't index data. <br> **See also**: [Agent connector](#a); [MCP](#m) |
...
| [MCP](../m365-apps/agent-connectors.md) | Model Context Protocol (MCP) is an open protocol that agents use to discover and call tools that an external server exposes; MCP is the protocol itself, not a Microsoft product or a specific connector type. Register a remote MCP server for Microsoft 365 agents as an [agent connector](#a), and for declarative agents as a [plugin](#p). <br> **See also**: [Agent connector](#a); [MCP server](#m); [Plugin](#p) |
| [MCP server](../m365-apps/agent-connectors.md) | A remote service that you build and host to expose tools through the Model Context Protocol (MCP) for agents to call; you register it as an [agent connector](#a) or a [plugin](#p), not as a [Copilot connector](#c). An MCP server isn't hosted by Microsoft 365, and registering one doesn't require Azure resources. <br> **See also**: [MCP](#m); [Agent connector](#a); [Plugin](#p) |
...
| [Message extension](../messaging-extensions/what-are-messaging-extensions.md) | Message extensions (previously called [composeExtensions](/microsoft-365/extensibility/schema/root-compose-extensions)) are shortcuts for inserting app content or acting on a message. You can use a message extension without navigating away from the conversation. <br>**See also**: [Search commands](#s); [Action commands](#a) |
...
| [Meeting surfaces](/microsoft-365/extensibility/schema/root#configurabletabs) | The set of `meetingSurfaceItem` scopes where a tab is supported. <br>**See also**: [Configurable tab](#c)|
...
| [Plugin](/microsoft-365-copilot/extensibility/overview-plugins) | A plugin lets a declarative agent for Microsoft 365 Copilot call a Model Context Protocol (MCP) server or a REST API that has an OpenAPI description. A plugin is declared in a plugin manifest that ships with the agent, which makes it different from an [agent connector](#a), declared in the app manifest. <br> **See also**: [Agent connector](#a); [Declarative agent](#d); [MCP](#m) |
...
| [Privacy URL](/microsoft-365/extensibility/schema/root-developer#privacyurl)| A property in the app manifest file (`privacyUrl`). The HTTPS URL to the page that provides privacy information for the app.|
...
| [Real-time meeting protocol](/microsoft-365/extensibility/schema/root-meeting-extension-definition)| A Boolean value indicating whether this app can stream the meeting's audio and video content to a Real-time meeting protocol (RTMP) endpoint.|
...
| [Subscription offer](/microsoft-365/extensibility/schema/root-subscription-offer) | This specifies the SaaS offer associated with your app. A unique identifier that includes your Publisher ID and Offer ID, which you can find in Partner Center. <br>**See also**: [SaaS](#s)|
...
| [Teams Toolkit](../toolkit/agents-toolkit-fundamentals.md) | The Teams Toolkit, now knows as Microsoft 365 Agents Toolkit, enables you to create custom Teams apps directly within the VS Code environment. <br> See [Microsoft 365 Agents Toolkit](#m) |
...
| [Transactable Teams SaaS offer](/microsoft-365/extensibility/schema/#root-subscription-offer) | Transactable Teams SaaS offer (T-SaaS) specifies the SaaS offer associated with your app. A unique identifier that includes your Publisher ID and Offer ID, which you can find in Partner Center. <br>**See also**: [SaaS](#s)|
...
| Term | Definition |
| --- | --- |
| [Valid domains](/microsoft-365/extensibility/schema/root#validdomains) | A property in the app manifest file (`validDomains`). A list of valid domains for websites the app expects to load within the supported clients. |

...
3. Register a `microsoftTeams.settings.registerOnSaveHandler()` event handler, which gets called when the user selects **Save**.
4. Call `microsoftTeams.settings.setSettings()` to save the connector settings. What is saved here is also shown in the configuration dialog if the user tries to update an existing configuration for your connector.
5. Call `microsoftTeams.settings.getSettings()` to fetch webhook properties, including the URL. You must call this during the save event and when your page is first loaded in the case of a re-configuration.
...
| `userObjectId` | This is the unique ID corresponding to the Office 365 user who initiated setup of the connector. It must be secured. This value can be used to associate the user in Office 365 who set up the configuration to the user in your service. |
...
Your code must handle users returning to edit an existing connector configuration. To do this, call `microsoftTeams.settings.setSettings()` during the initial configuration with the following parameters:
...
Typically, this call is made as part of your save event handler. Then, when the `contentUrl` is loaded, your code must call `getSettings()` to pre-populate any settings or forms in your configuration UI.
...
Sometimes, you may not want to publish your connector app to the public AppSource or Store but would like it to be available only to the users in your organization. In such cases, you can upload your custom connector app to your [organization's App Catalog](~/concepts/deploy-and-publish/apps-publish.md). This way, your connector app is available only to that organization and you need not publish your connector to the public store.
...
1. Select the connector that you want to add and a pop-up dialog window displays.
...
1. The connector is available in the  section &#9679;&#9679;&#9679; > **More options** > **Connectors** > **All** > **Connectors for you team** for that team. You can navigate by scrolling to this section or search for the connector app.

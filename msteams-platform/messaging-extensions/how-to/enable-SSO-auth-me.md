...
Single sign-on support is now available for messaging extensions and link unfurling. When you enable the Single sign-on (SSO), the messaging extensions silently refresh the authentication token. Further, the authentication token minimizes you to enter sign-in credentials for Microsoft Teams.
...
4. You receive the token in `OnTeamsMessagingExtensionQueryAsync` handler in the `turnContext.Activity.Value` payload or in the `OnTeamsAppBasedLinkQueryAsync`, depending on which scenario you enable the SSO:
    ...
    While using the OAuth connection, add the following code to the TeamsMessagingExtensionsSearchAuthConfigBot.cs file to update or add the token in the store:

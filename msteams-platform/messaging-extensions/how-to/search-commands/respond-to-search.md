...
* Extracted from the basic `title`, `text`, and `image` properties of the `attachment` object. The basic properties are used only if the `preview` property isn't specified.
* The Hero or Thumbnail card button and tap actions, except invoke, aren't supported in the preview card.

To send an Adaptive Card or O365 connector card, you must include a preview. The `preview` property must be a hero or thumbnail card. If you don't specify the preview property in the `attachment` object, a preview isn't generated.

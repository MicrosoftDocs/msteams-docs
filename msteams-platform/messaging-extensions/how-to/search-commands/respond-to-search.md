...
The following code snippet shows the `config` response that appears when the user interacts with the compose extension:
...
* The `type` field within `composeExtension` that's set to `config`, indicating the nature of this response as a configuration. 

...
The result is displayed in the Microsoft Teams UI with a preview of each item. The preview is generated in one of the two ways:

* Using the `preview` property within the `attachment` object. The `preview` attachment can only be a Hero or Thumbnail card.
* Using the basic `title`, `text`, and `image` properties of the `attachment` object. The properties are used only if the `preview` property isn't set and are available.
* The Hero or Thumbnail card button and tap actions, except invoke, aren't supported in the preview card.

To send an Adaptive Card or O365 connector card, you must include a preview. The `preview` property must be a hero or thumbnail card. If you don't specify a preview property in the `attachment` object, a preview isn't generated.

For Hero and thumbnail cards, you don't need to specify a preview property, a preview is generated by default. The following example displays link unfurling feature when a link is pasted in messaging extension:  

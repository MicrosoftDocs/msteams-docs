...
Your service should verify that the authentication code received in step 6 matches with step 5. It ensures that a malicious user doesn't try to spoof or compromise the sign in flow, which effectively "closes the loop" to finish the secure authentication sequence.

...
Your service should verify the authentication code received in step 6 matches the one from step 5. It ensures that a malicious user doesn't try to spoof or compromise the sign in flow. It effectively "closes the loop" to finish the secure authentication sequence.

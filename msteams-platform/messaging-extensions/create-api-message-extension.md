...
From command palette, click Teams: Create a New App -> Message Extension -> Custom Search Result -> Start with an OpenAPI Description Document -> Enter OpenAPI Description Document Location -> Input: <https://raw.githubusercontent.com/SLdragon/example-openapi-spec/main/multiparam.yml> -> select GET /pet/findByStatus API and click OK -> Default folder -> Input an App name -> Enter
Provision the project
Trigger Preview in Teams
Input PetId: 4, Test: 5, then click search, you will see the result (if this PetId is not work, use another id and try again)
...
1. Select **Search**. The Adaptive card appears in the message compose box.

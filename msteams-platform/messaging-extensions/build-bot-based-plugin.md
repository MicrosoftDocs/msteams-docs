...
The fields such as app name, command ID, command title, command description, parameter name, and parameter description enable the Copilot to identify the appropriate plugin and extract the parameters or commands. If your description strings exceed 128 characters, we recommend you to  use `Command.semanticDescription` and `Parameter.semanticDescription` to provide the descriptions to Copilot. For more information, see [define descriptions](high-quality-message-extension.md#define-descriptions).

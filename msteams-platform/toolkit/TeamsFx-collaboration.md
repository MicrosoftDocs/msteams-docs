...
### As project owner
  ...
  > [!NOTE]
  > Before adding collaborators for an environment, the project owner needs to [provision](provision.md) the project first.
  ...
  1. In the **ENVIRONMENT** section, select the Manage Collaborators button as shown in the following image:
  ...
  1. Select **Add App Owners** to add another Microsoft 365 account email address as a collaborator:
        ...
        * (Optional) Select and confirm the Teams `manifest.json` file.
        ...
        * (Optional) Select and confirm the Microsoft Entra app `aad.manifest.json` file.
  ...
  1. Select **List App Owners** to view collaborators in the current environment:
...
### As project collaborator
  ...
  5. Launch remote to have a preview of the Teams app.
     ...
     > [!NOTE]
     > Collaborators must log in using the account that the project owner adds under the same tenant with the project owner. For more information, see [build and run your Teams app in a remote environment](/microsoftteams/platform/sbs-gs-javascript?tabs=vscode%2Cvsc%2Cviscode%2Cvcode&tutorial-step=3).
...
* Using Microsoft Entra ID
  ...
  * Go to [Microsoft Entra ID](https://ms.portal.azure.com/#blade/Microsoft_AAD_IAM/ActiveDirectoryMenuBlade/RegisteredApps), select **App registrations** from the left panel, and find your app.
  ...
  * Select and remove the owner.
    ...
    > * Azure-related permissions must be set manually by the Azure subscription administrator on the Azure portal. To manually remove the permissions, see [revoke granted consent to permissions](/microsoftteams/manage-consent-app-permissions#revoke-granted-consent-to-permissions).
    > * The Azure account must have the [contributor role](/azure/role-based-access-control/built-in-roles#contributor) for the subscription so that developers can work together to provision and deploy the TeamsFx project.

...
|---------|---------|--------------------------|
|Bot and message extension    |A bot template code is added into a subfolder with path `yourProjectFolder/bot`.</br></br>Files `launch.json` and `task.json` under `.vscode` folder are updated.</br></br>File`manifest.template.json` under `templates/appPackage` folder is updated. The changes are visible in ID of your bot, scopes of your bot, and The commands that hello world bot application can respond to.</br></br>Files under `templates/azure/teamsfx` are updated, and `templates/azure/provision/xxx`.bicep files are regenerated.</br></br>Files under `.fx/config` are regenerated|Includes a **hello world** bot application template into your project.</br></br>Includes necessary scripts for Visual Studio Code, and is executed when you want to debug your application locally.</br></br>Includes bot related information in the manifest file that represents your application in Teams Platform.</br></br>Includes</br></br>Ensures your project is set with right configurations for newly added capability. |

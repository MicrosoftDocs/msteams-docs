...
1. To test your bot in mobile client, follow the steps listed in [Test your tab app on mobile client](#test-your-tab-app-on-mobile-client) for your bot.
    ...
    > [!NOTE]
    > If a you have debugged the bot app previously and the app manifest (previously called Teams app manifest) file is changed, it's recommended to clear the cache on the mobile device to ensure immediate app synchronization. After clearing the cache, the app takes some time to sync.

---
title: Debug bot using Microsoft 365 Agents Playground
author: surbhigupta 
...
> [!NOTE]
>
> Microsoft 365 Agents Playground is available in the latest prerelease version of the Microsoft Teams Toolkit. Ensure that you install the [latest prerelease version](install-Teams-Toolkit.md#install-a-prerelease-version) of the Teams Toolkit.

Microsoft 365 Agents Playground makes debugging bot-based apps effortless. You can chat with your bot and see its messages and Adaptive Cards as they appear in Microsoft Teams. You don’t need a Microsoft 365 developer account, tunneling, or Teams app and bot registration to use Microsoft 365 Agents Playground.

The following image shows a sample app displaying an Adaptive Card with a list of commands in Microsoft 365 Agents Playground. It also provides a description of the commands so that you can test your app without manually searching your code:

   :::image type="content" source="../assets/images/teams-toolkit-v2/debug/sample-app-output.png" alt-text="Screenshot shows Microsoft 365 Agents Playground Adaptive Card.":::

The following are the advantages of Microsoft 365 Agents Playground:

* **Sandbox environment**: The sandbox environment of Microsoft 365 Agents Playground emulates the behavior, look, and user experience of Teams.

* **Tunneling**: An external tunnel service isn't necessary as Microsoft 365 Agents Playground runs on a local server that your bot can communicate with.
...
* **Mock data and activities**: Microsoft 365 Agents Playground makes it easy to test complex scenarios such as, sending a welcome message when a new member joins the channel, using mock data and activity triggers.

* **Reliable**: Microsoft 365 Agents Playground is reliable as the bot's Adaptive Card utilizes the same rendering technology as in Teams.

* **Integration with existing Teams bot applications**: Microsoft 365 Agents Playground integrates effortlessly with existing Teams bot applications built with Bot Framework SDK.

* **Support for different scopes**: Microsoft 365 Agents Playground supports testing in personal, team, and group chat scopes.

## Prerequisites

Ensure you install the following tools for building and deploying your bots in Microsoft 365 Agents Playground:
...
## Understand Microsoft 365 Agents Playground

Microsoft 365 Agents Playground is an npm package that has a CLI command called `teamsapptester`. When you run `teamsapptester start`, it opens a web app on your local machine that emulates the Teams web client and Bot Framework service. This web app doesn't need any cloud resources as it uses mock data to simulate the contextual information of Teams.

To use a Teams bot on Microsoft 365 Agents Playground, you need to provide:

* Message endpoint: A bot message endpoint is the URL that links Microsoft 365 Agents Playground and your bot. You can update the endpoint with the `BOT_ENDPOINT` environment variable or use the default value of `http://localhost:3978/api/messages`.
* Configuration file (Optional): A configuration file informs Microsoft 365 Agents Playground about your customized contextual information in Teams. The file is named **.teamsapptesttool.yml** in the project's root folder. If Teams can't find this file, it uses the default configuration. For more information, see [customize Teams context](#customize-teams-context).

## Microsoft 365 Agents Playground experience in Teams Toolkit

Microsoft 365 Agents Playground offers a faster debug experience for bot applications when compared to the Teams client.
...
1. Microsoft 365 Agents Playground opens the bot in a webpage.
   ...
   :::image type="content" source="../assets/images/teams-toolkit-v2/debug/test-tool.png" lightbox="../assets/images/teams-toolkit-v2/debug/test-tool.png" alt-text="Screenshot shows the bot open in Microsoft 365 Agents Playground.":::
...
1. You can use command line for JavaScript and TypeScript or C# to debug your app in Microsoft 365 Agents Playground. To get started select the following language:

   # [JavaScript/TypeScript](#tab/clijs)

   1. Run the following command to install Microsoft 365 Agents Playground CLI from `npm`:
   ...
   1. Run the following command in a separate terminal to launch Microsoft 365 Agents Playground:
   ...
   1. Download Microsoft 365 Agents Playground CLI from the [GitHub](https://github.com/OfficeDev/TeamsFx/releases?q=teams-app-test-tool&expanded=true) release.
   ...
   1. Run the following command to initiate Microsoft 365 Agents Playground:
      ...
      If the Microsoft 365 Agents Playground for C# doesn't initiate because of a port conflict, alter the Microsoft 365 Agents Playground's port number in the `TEAMSAPPTESTER_PORT` environment variable where you run `teamsapptester.exe start`.
     ---
   1. Microsoft 365 Agents Playground opens the bot in a webpage.
...
You can mock an activity in Microsoft 365 Agents Playground using activity triggers. There are two types of activity triggers:
...
Microsoft 365 Agents Playground provides predefined activity triggers to test the functionalities of your bot.
...
Predefined activity triggers are available in the **Mock an Activity** menu in Microsoft 365 Agents Playground.

To mock an **Add user** activity, follow these steps:

1. In Microsoft 365 Agents Playground, go to **Mock an Activity** and select **Add user**.
...
You can use **Custom activity** to customize activity triggers such as, `reactionsAdded`, to fit the requirements of your bot app. Microsoft 365 Agents Playground automatically populates the required properties of the activity. You can also modify the activity type and add more properties.
...
<details><summary>Microsoft 365 Agents Playground contains a built-in configuration file in the project's root folder.</summary>
...
To comprehensively test this bot in Microsoft 365 Agents Playground, it's crucial to update the configuration file to use the correct email addresses of the owners of the inactive bugs.
...
1. Save the file and select **F5** to debug in Microsoft 365 Agents Playground.
...
* It impacts the user interface in Microsoft 365 Agents Playground, for example, group chat names.

## Limitations

* Bot features enabled through the Teams app manifest aren't available as Microsoft 365 Agents Playground doesn't process it.

* Microsoft 365 Agents Playground doesn't support all types of cards except Adaptive Cards.

* Microsoft 365 Agents Playground doesn't support the following Adaptive Card features:
...
* Microsoft 365 Agents Playground doesn't support the following experiences:

  * Mobile
  * Meeting

* Microsoft 365 Agents Playground can emulate the following experiences:

   | Features | Debug in Microsoft 365 Agents Playground | [Debug your Teams app locally](debug-local.md) |
   | --- | --- | --- |
...
## Debug an existing app with Microsoft 365 Agents Playground

Ensure you have an existing bot created using Teams Toolkit. To debug your bot with Microsoft 365 Agents Playground, follow these steps:
...
Microsoft 365 Agents Playground successfully debugs your existing bot.
...
<details>
<summary>How can I test my bot if Microsoft 365 Agents Playground doesn't support its features?</summary>

You can always use the Teams client to test the features that Microsoft 365 Agents Playground doesn't support. Select the option **Debug in Teams (Edge)** or **Debug in Teams (Chrome)** to test your application in the Teams client.
<br>
&nbsp;
</details>
<details>
<summary>How would I know if Microsoft 365 Agents Playground doesn't support features in my bot?</summary>

Microsoft 365 Agents Playground shows a warning message in conversation and log panel when it detects unsupported features.
...
<details>
<summary>Does Microsoft recommend using only Microsoft 365 Agents Playground for testing bot applications?</summary>
...
|----------------|-----------------|--------------|
| Microsoft 365 Agents Playground Sample App | A sample bot app to explore Microsoft 365 Agents Playground. | [View](https://github.com/OfficeDev/TeamsFx-Samples/tree/v3/test-tool-sample-app) |

## Step-by-step guide

Follow the [step-by-step guide](../sbs-teams-app-test-tool.yml) to debug an AI chat bot using Microsoft 365 Agents Playground.

...
1. Add the downloaded app manifest template code to `./aad.manifest.json` file. This allows you to customize different aspects of your app registration and update the manifest as required. For more information, see [Entra app manifest](/entra/identity-platform/reference-app-manifest).
...
To update source code for an agent:
  ...
  > [!NOTE]
  > If you're adding SSO to agents built with C#, TypeScript, or Python, see [Teams SDK app authentication](/microsoftteams/platform/teams-sdk/essentials/app-authentication?pivots=csharp).
...
You can update the source code for the following Teams apps to add SSO:

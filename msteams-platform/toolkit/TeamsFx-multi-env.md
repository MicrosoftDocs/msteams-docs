...
> [!NOTE]
> If you have more than one existing environments, you need to select an existing environment to create the same. The command copies file contents of `config.<newEnv>.json` and `azure.parameters.<newEnv>.json` from the existing environment you selected to the new environment being created.
...
The following table lists the common scenarios supported for customized provision and the location to customize:

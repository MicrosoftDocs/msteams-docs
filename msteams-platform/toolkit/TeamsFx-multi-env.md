...
After you create a project, Teams Toolkit configures the following default environments:
...
> [!Note]
> If you have more than one environment, you need to select an existing environment to create the new environment. The command copies the file contents of `config.<newEnv>.json` and `azure.parameters.<newEnv>.json` from the existing environment you've selected to the new environment that you’re creating.
...
6. Run provision command on **dev** and **staging** environment to update the app name in remote environments.
...
You can define Teams app description for the each environments:

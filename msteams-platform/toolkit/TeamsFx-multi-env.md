...
3. Select **+ Create new environment** under **ENVIRONMENT**.
   ...
   If you have more than one environment, you need to select an existing environment to create the new environment. The command copies the file contents of `config.<newEnv>.json` and `azure.parameters.<newEnv>.json` from the selected environment to the new environment that you’re creating.
...
* `.fx\states`: Provision output that is generated by Teams Toolkit after you provision resources for your app.

...
author: timtwang
ms.author: mobajemu
description: Tutorial for integrating Fluid-powered real-time collaboration features into a Microsoft Teams Launch Page
...
This is a tutorial for integrating Fluid-powered real-time collaboration features into a [Microsoft Teams Launch Page](../msteams-platform/tabs/what-are-tabs). By the end of this tutorial you will be able to integrate any Fluid-powered applications into Microsoft Teams and collaborate with others in real-time.

Concepts you will learn:
1. How to integrate a Fluid client into a Microsoft Teams Launch Pages
1. How to run and connect your Teams application to a Fluid service (Azure Fluid Relay)
1. How to create and get Fluid Containers and pass them to a React component
...
2. Create a vanilla Teams Launch Page by running the below command. See [the Microsoft Teams documentation](../msteams-platform/tabs/how-to/create-channel-group-tab?tabs=nodejs) for more information on setting up Teams applications.
...
1. Replace `TODO: 3` with the following code. Before the client can be used, it needs an `AzureClientProps` that will define the type of connection the client will be using. Think of the `connectionConfig` as the properties required to connect to the service. Note that local mode of Azure Client is used here. To enable collaboration across all client, please replace it with Fluid Relay Service credentials. See how to [set up the Azure Fluid Relay service](../azure-docs/articles/azure-fluid-relay/how-tos/provision-fluid-azure-portal) for more information.
...
1. Replace `TODO 1` with the following code. For the content page to display in Teams, you must include the [Microsoft Teams JavaScript client SDK](../msteams-client-docs-ref-typescript/nodejs/overview/msteams-client.md) and include a call to initialize it after your page loads.
...
1. Replace `TODO 3` with the following code. Using a React state for the container will provide the ability to dynamically update the container and the data objects inside it.
...
To run and start the Teams application, open another terminal and follow the [instructions to run the application server](../msteams-platform/tabs/how-to/create-channel-group-tab?tabs=nodejs#upload-your-application-to-teams).

Now follow the [instructions to upload the application to a Teams Tab](../msteams-platform/tabs/how-to/create-channel-group-tab?tabs=nodejs#upload-your-application-to-teams).

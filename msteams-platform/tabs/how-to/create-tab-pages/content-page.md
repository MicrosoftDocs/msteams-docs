...
1. As a **mandatory** step, call `app.notifySuccess()` in all iframe-based contents to notify Teams that your app has successfully loaded. Then, Teams hides the loading indicator, if applicable. If `notifySuccess`  isn't called within 30 seconds, Teams assumes that your app timed out, and displays an error screen with a retry option. For App updates, this is applicable for already configured tabs as well, failing to do so will result in error screen for existing users. 

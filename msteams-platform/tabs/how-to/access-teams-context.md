...
The `theme` argument in the function is a string with a value of `default`, `dark`, `glass`, or `contrast`.  The [AppInfo](/javascript/api/%40microsoft/teams-js/app.appinfo?branch=main&view=msteams-client-js-latest#@microsoft-teams-js-app-appinfo-theme&preview-view=true) property in the getContext API supports the `glass` theme for Teams running on Apple Vision Pro devices.

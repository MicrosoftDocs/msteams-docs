...
> - If you've two separate apps registered in Microsoft Entra ID and require individual tokens for each app, use the OBO flow to enable communication between the apps.
...
1. The token retrieved using `getAuthToken()` must be exchanged on the server-side using Microsoft Entra [OBO flow](/azure/active-directory/develop/v2-oauth2-on-behalf-of-flow) to get access to those other Graph APIs. Ensure that you use the Microsoft Entra v2 endpoint for this exchange.

...
If you create a bot with a **personal** scope, it appears in the first tab position in a personal app by default. If you want to move it to another position, you must add a static tab object to your manifest with the reserved keyword, **conversations**. The **conversation** tab appears on web, desktop, or mobile depending on where you add the **conversation** tab in the `staticTabs` array.
...
```
This property also enables you to set the default landing capability for your app, enabling the app to open as a bot or tab by default. For more information, see [Configure default landing capability](../../concepts/deploy-and-publish/add-default-install-scope.md#configure-your-apps-default-landing-capability).

...
Stage View improves user experience and is designed to:
...
Multi-window Stage View is useful for scenarios that require a user to multitask in Teams without the need for collaboration. The view opens your content in a new window without a side-panel conversation, allowing your user to focus on the singular task at hand. We recommend Multi-window Stage View when user isn't opening content from personal app.
...
* Collaborative Stage View from Adaptive Card
* StageView API
* Deeplink
...
| ---| ---| --- |
| Adaptive Card | Opens in Collaborative Stage View | Opens in Stage View modal, if Collaborative Stage View or Teams Multi-window isn't supported. |
...
* If Collaborative Stage View is invoked via Adaptive Card JSON from the web client, a Stage View Modal is opened instead, as multi-window Microsoft Teams experiences aren't yet supported in the browser.
...
The default behavior of Stage View deeplinks is to open in Collaborative Stage View with an associated side panel conversation. To invoke the deeplink from your tab or personal app, you must wrap the deeplink URL in the app.openLink(url) API. The `openMode` parameters defined in the API determine the way the chat content opens.
...
> * All deeplinks must be encoded before pasting the URL. Unencoded URLs are not supported.
...
Whether you want to facilitate multitasking, enhance collaboration, or provide a focused user experience, the Stage View Multi-window has a mode to suit your needs. Remember to consider the context in which your app is being used and ensure that your app works in that context.

...
var useVirtualTree_1 = require("../../../../src/components/Tree/hooks/useVirtualTree");
...
describe('useVirtualTree', function () {
    var useVirtualTreeResult;
    beforeAll(function () {
        testHook(function () {
            useVirtualTreeResult = useVirtualTree_1.useVirtualTree({
                items: items,
                defaultActiveItemIds: ['1'],
            });
        });
    });
    test('should return correct flatTree', function () {
        expect(useVirtualTreeResult.flatTree).toStrictEqual(flatTreeItems);
    });
    test('should return correct activeItemIds and visibleItemIds', function () {
        expect(useVirtualTreeResult.activeItemIds).toEqual(['1']);
        expect(useVirtualTreeResult.visibleItemIds).toEqual(['1', '11', '12', '2', '3']);
    });
    test('should return getItemById', function () {
    ...
    test('should return registerItemRef and getItemRef', function () {
        var dummyElement = document.createElement('div');
        useVirtualTreeResult.registerItemRef('1', dummyElement);
        expect(useVirtualTreeResult.getItemRef('1')).toBe(dummyElement);
    });
    test('should return ref to be assigned to virtual list', function () {
        expect(useVirtualTreeResult.listRef).not.toBeUndefined();
    });
});
describe('useVirtualTree with state change', function () {
    var useVirtualTreeResult;
    beforeEach(function () {
        testHook(function () {
            useVirtualTreeResult = useVirtualTree_1.useVirtualTree({
    ...
    test('should update activeItemIds/visibleItemIds when toggleItemActive is called', function () {
        test_utils_1.act(function () {
            useVirtualTreeResult.toggleItemActive({}, '12');
        });
        expect(useVirtualTreeResult.activeItemIds).toEqual(['1', '12']);
        expect(useVirtualTreeResult.visibleItemIds).toEqual(['1', '11', '12', '121', '2', '3']);
    });
    test('should update activeItemIds/visibleItemIds when expandSiblings is called', function () {
        test_utils_1.act(function () {
            useVirtualTreeResult.toggleItemActive({}, '12');
            useVirtualTreeResult.expandSiblings({}, '1');
        });

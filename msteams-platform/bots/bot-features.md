...
An extensive dialog requires the developer to maintain state. To exit this state a user must either time out or select **Cancel**. In addition, the procedure is time consuming. Consider the following example of a conversation:
...
As there are only six visible commands, adding more is unlikely to be used frequently. Bots that focus on a single task rather than attempting to be a general assistant perform better.

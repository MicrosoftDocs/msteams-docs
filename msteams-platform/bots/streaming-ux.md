...
* **Citations**: Teams AI library automatically enables citations for the response that the bot message generates. It adds the references for the source data that the bot has used to generating the response.
* **Sensitivity Label**: You can define the sensitivity label using the `StreamingResponse` class.
* **Feedback Loop**:You can set the feedback loop using the `AIOptions` object in the app devlaration and specify a handler.
* **Generated by AI**: Teams AI library automatically adds the Generation by AI tag for the response that the bot message generates.

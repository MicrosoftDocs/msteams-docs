...
description: Learn how to enhance the user experience in bots using streaming techniques and to stream messages through Teams SDK and configure streaming bot messages.
ms.date: 02/19/2025
ms.topic: conceptual
...
>[!NOTE]
>
> Streaming bot messages are available:
>
> - Only for one-on-one chats.
> - Generally available on web, desktop, and mobile.
...
Streaming bot messages have two types of updates:
...
Once your bot is ready to deliver its final message to the user, transition from providing informative updates to response streaming. For each response streaming update, ensure the message content reflects the latest version of the final message. This requires your bot to incorporate any new tokens generated by the Large Language Models (LLMs). Append these tokens to the previous message version before sending it to the user.

The throttling limit is set at 1 request per second. Ensure the bot sends requests within this limit. The bot may send requests at a slower rate if necessary.
...
Once your bot completes generating its message, send the end streaming signal along with the final message. For the final message, the `type` of activity is `message`. Here, the bot sets any fields that are allowed for the regular message activity but `final` is the only allowed value for `streamType`.

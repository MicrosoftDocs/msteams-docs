...
The app template is built using the Microsoft 365 Agents SDK (previously known as TeamsFx SDK), which provides a simple set of functions over Microsoft Bot Framework to implement your requirement. Refer [documentation](/microsoft-365/agents-sdk/) for more details. <!--For example, a travel agency builds an app in Teams for their users to keep them up to date with the weather forecast. In the following flowchart, a Teams app notifies about the weather forecast to the users using an Adaptive Card:<br>:::image type="content" source="../assets/images/notification-bot/notification-new-scenario-diagram.png" alt-text="Image shows a weather forecast sample notification scenario" lightbox="../assets/images/notification-bot/notification-new-scenario-diagram.png":::-->
...
- Facilitates notifications to a personal, group chat, and in a channel, using APIs from Microsoft 365 Agents SDK (previously known as TeamsFx SDK).
...
Bot Framework SDK provides the functionality to proactively message in Teams. Microsoft 365 Agents SDK (previously known as TeamsFx SDK) provides the functionality to manage bot's conversation references when a bot event is triggered. Microsoft 365 Agents SDK recognizes the following bot events:
...
When you send notifications, Microsoft 365 Agents SDK (previously known as TeamsFx SDK) creates a new conversation from the selected conversation reference, and then sends a message. For advanced usage, you can directly access the conversation reference to execute your own bot logic:
...
Microsoft 365 Agents SDK (previously known as TeamsFx) supports two ways to help you send notifications from your system to Teams:
...
Microsoft 365 Agents SDK (previously known as TeamsFx) provides you with an [Incoming Webhook notification sample](https://github.com/OfficeDev/TeamsFx-Samples/tree/v3/incoming-webhook-notification) that helps you to:

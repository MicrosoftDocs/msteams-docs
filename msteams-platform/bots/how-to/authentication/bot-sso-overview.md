...
1. **Update app manifest (previously called Teams app manifest)**: Update your Teams client app manifest with the app ID and application ID URI generated in Azure AD to allow Teams to request access tokens on behalf of your app. The update required in the app manifest file is the same for bot and message extension apps.

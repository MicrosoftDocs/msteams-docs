...
8. In the panel that opens, add a client app by entering `access_as_user` as the **Scope name**.
    ...
    >
    > * Only user-level Microsoft Graph API permissions, such as email, profile, offline_access, and OpenId are supported. If you need access to other Microsoft Graph scopes, such as `User.Read` or `Mail.Read`, see [Acquire an access token with Graph permissions](../../../tabs/how-to/authentication/auth-aad-sso.md#acquire-an-access-token-with-graph-permissions).

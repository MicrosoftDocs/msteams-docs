...
2. If the current user has used your bot application for the first time, the user needs to provide consent (if consent is required) to the request prompt or handle step-up authentication (such as two-factor authentication).
...
1. Specify the permissions that your application needs for the Azure AD endpoint and, optionally, for Microsoft Graph.
...
The following code is required to add new properties to your Teams manifest:
...
3. The following steps are required to complete the **New Connection Setting** form:
    ...
    2. In the **Service Provider** drop-down, select **Azure Active Directory V2**.
...
1. Open the [Teams auth sample](https://github.com/microsoft/BotBuilder-Samples/tree/master/samples/csharp_dotnetcore/46.teams-auth).

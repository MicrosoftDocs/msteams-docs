...
1. Enter the appropriate Microsoft 365 client ID for the applications that you want to authorize for your app’s web application.
    ...
    > * You can use a different App Registration ID in the webApplicationInfo section of your Teams app manifest that's not same as the Azure Bot's App ID. This is a valid and supported configuration.
    >    * The `botId` in the bots section refers to the Azure Bot registration used for messaging and interaction.
    >    * The `webApplicationInfo.id` refers to the Azure AD App Registration used for authentication such as SSO, token acquisition, Graph API access.

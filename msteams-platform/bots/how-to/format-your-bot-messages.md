...
> [!NOTE]
> The bot message size limit is 40 KB. If bot message size limit exceeds 40 KB, bot will receive a status code of `413` (RequestEntityTooLarge) with message containing error code `MessageSizeTooBig`. The bot message size limit includes the entire message payload encoded as UTF-16.

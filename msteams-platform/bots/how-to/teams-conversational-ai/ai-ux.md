...
An agent transforms system interactions. For developers, creating an exceptional user experience is crucial. This article details the steps, principles, and considerations for designing intuitive, user-centered interfaces that seamlessly integrate AI capabilities. The main goals are to simplify complex tasks, enhance productivity, and offer personalized experiences through adaptive learning. An agent includes features that enhance its functionality and integration within Teams environment:
...
- [Update app manifest an agent](#update-the-app-manifest-an-agent)
- [Stream agent response to user](#stream-the-agent-response-to-the-user)
- [Ensure agent response contains citations](#ensure-the-agent-response-contains-citations)
- [Ensure agent response contains an AI label](#ensure-the-agent-response-contains-an-ai-label)
- [Ensure that agent maintains intelligent conversation](#ensure-that-the-agent-maintains-intelligent-conversation)
- [Ensure that agent offers prompt starters or a welcome card](#ensure-that-the-agent-offers-prompt-starters-or-a-welcome-card)

### Update app manifest an agent

You must update app manifest for the agent to define specific properties and configurations that characterize its capabilities and behavior.

Here's an example for updating app manifest. You must add `botID` property to `copilotAgents` node in app manifest.
...
Use Teams SDK to add streaming to the agent.

### Ensure agent response contains citations
...
### Ensure agent response contains an AI label
...
### Ensure that agent maintains intelligent conversation
...
### Ensure that agent offers prompt starters or a welcome card
...
- [Enable agent to understand conversational history and context](#enable-the-agent-to-understand-conversational-history-and-context).
...
- Provide feedback received from users to the agent.
- Use feedback to improve the quality of agent's responses.
...
Design your agent to understand and refer to conversational history and context. It ensures that every interaction is relevant and tailored to user's specific needs. The agent refers to the context and offers responses that are accurate and contextually appropriate.
...
Enhance your agent's user experience with intelligent and context-aware prompts. The agent offers context-relevant prompts dynamically.
...
To enable app profile card to your agents or bots, add the `features` field under the `description` field in the app manifest. For more information, see [public developer preview for Teams](/microsoft-365/extensibility/schema/root-description?view=m365-app-prev&tabs=syntax&preserve-view=true).

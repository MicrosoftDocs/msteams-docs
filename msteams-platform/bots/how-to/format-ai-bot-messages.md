...
The bot receives the user's input, received in the feedback form, through a bot invoke flow. If your bot is built using Teams AI library, the bot invoke received is automatically handled. To handle feedback, use the `app.feedbackLoop` method to register a feedback loop handler that is called when the user provides feedback.
...
If your bot is built using Bot Framework SDK, you need to have an `onInvokeActivity` handler to correctly process the feedback. Ensure that you return a status code 200 with an empty JSON object as a response.

...
An AI label on your bot message indicates it’s generated by AI. While AI bots using Large Language Models (LLMs) are generally reliable, the label helps prevent confusion about the information's source.
...
After adding an AI label, your bot’s message will show an "AI generated" label next to the bot’s name. A disclaimer hovers stating, "AI-generated content may be incorrect." The AI label and disclaimer cannot be customized for AI-powered bots.
...
The bot receives user input from the feedback form through a bot invoke flow. For bots built using the Teams AI library, the bot invoke request is automatically handled. To manage feedback, use the app.feedbackLoop method to register a feedback loop handler when the user provides feedback.

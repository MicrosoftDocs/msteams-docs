...
1. In Visual Studio Code, open `src/main.py`. Find the line that reads `app = App()` and modify it to read:
...
1. Start the agent runtime. Once it's running, you'll see a confirmation that it's running on port 3978.
...
1. Close the Agents Playground tab in your browser, and use <kbd>Ctrl+C</kbd> in both console windows to stop Agents Playground and your agent runtime. Leave open the console window you used to create your agent runtime code, you'll need it again soon.
...
The CLI will confirm your account's permissions to install custom apps ("sideloading") and check for the presence of an Azure CLI installation (not required for this quickstart).

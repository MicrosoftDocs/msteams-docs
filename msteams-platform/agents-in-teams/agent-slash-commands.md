...
User-to-agent [targeted messaging](targeted-messages.md) (one-to-one private messaging in group conversations) is surfaced to users as a slash command named for the agent. Activating one of these agent-named commands switches the compose box to targeted message mode for the agent and displays a notice that the message will be sent privately. After the user composes a message and selects **Send**, the resulting message will be targeted to the agent and can't be seen by other participants in the conversation.

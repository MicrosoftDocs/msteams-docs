...
Use `slash` commands for quick discovery, private command invocation, and fast access from an empty compose box, especially for frequent actions and lightweight workflows. Use `mention` commands when users are intentionally addressing the agent as part of a visible interaction; support both triggers when the same command should be accessible through either entry point without changing its behaviour.

...
Apps and agents can expose four types of commands via their configuration:
...
All four types are supported across all client platforms, including mobile.

### Targeted messaging

User-to-agent [targeted messaging](targeted-messages.md) (one-to-one private messaging in group conversations) is surfaced to users as a slash command named for the agent. Activating one of these agent-named commands switches the compose box to targeted message mode for the agent, displaying a notice that the message will be sent privately. After the user composes a message and selects **Send**, the resulting message will be targeted to the agent and only visible to the agent and the sending user.

TODO screenshot

See [Send and receive targeted (private) messages in group conversations](targeted-messages.md) for information about targeted messaging, including how to configure an agent to be able to receive targeted messages.

### Conversational slash commands

Conversational slash commands are named commands that agents can surface to users via the slash command autocomplete menu. When a user opens the menu, it will contain all slash commands registered by all agents in the current conversation. Each command is displayed with the name and icon of the agent that provides them, enabling users to identify the associated agent and differentiate between commands with the same name.

When a user activates a conversational slash command provided by an agent, Teams switches the compose box to targeted messaging mode for the agent and autopopulates the name of the command as the message text. Selecting **Send** sends the targeted message to the agent.
...
Conversational slash commands rely on targeted messaging, and require that the agent is opted in to receive targeted messages; see [Receive targeted messages](targeted-messages.md#receive-targeted-messages).

### Conversational @mention commands

Conversational @mention commands work like conversational slash commands, but are exposed via a separate autocomplete box that appears after the user inserts an @mention of the agent in the compose box. When a user activates an @mention command, Teams appends the command name into the compose box as message text. The message that results from selecting **Send** is a public message containing an @mention shown to all participants in the conversation.

...
> [!NOTE]
> This API provides AI Insights for all meetings and calls for which a recap is available. This includes scheduled private and channel meetings, one-on-one and group VOIP calls,, webinars and ad-hoc meetings.
...
1. In case of one-on-one unscheduled teams or VOIP calls, each AI insight object provides detailed meeting notes, action items, and participant-specific mentions, which can be accessed by calling [GET AI Insights API](/graph/api/callaiinsight-get?view=graph-rest-beta&preserve-view=true) for a specific insight object ID only after the meeting is over. Here's an example request and response:
   ...
   **Request**
    ...
    **Response**
    ...
    | --- | --- |
    | `id` | A unique identifier for the generated AI insight object. |
    | `transcriptContentUrl` | A specific endpoint in the Microsoft Graph API to to retrieve the actual content of a transcript generated during the ad hoc call. |
    | `contentCorrelationId` | A unique identifier that correlates the [transcript](/graph/api/resources/calltranscript?&view=graph-rest-beta&preserve-view=true) of the meeting from which the AI insight object is generated. |
    | `createdDateTime` | The date and time at which the corresponding transcript was created. The timestamp type represents the date and time information using the ISO 8601 format and is always in Coordinated Universal Time (UTC). |
    | `endDateTime` | The date and time at which the corresponding transcript ends. The timestamp type represents the date and time information using the ISO 8601 format and is always in UTC. |

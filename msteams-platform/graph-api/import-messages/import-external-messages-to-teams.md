...
> [!NOTE]
>
> Federated content can't be imported. All imported content must come from the authenticated tenant and the application that calls `startMigration` on a thread owns that migration session end-to-end. The same app must invoke `importMessage` and `completeMigration`. No other app can call these APIs on the thread until the owning app completes migration.

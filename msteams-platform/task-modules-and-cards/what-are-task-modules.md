...
Task modules build on the foundation of Microsoft Teams tabs. The task modules are essentially a tab inside a pop-up window. Also, use the same SDK, so if user has built a tab then user is already familiar with creating a task module.
...
* Bots: Using buttons on [cards](~/task-modules-and-cards/cards/cards-reference.md) sent from your bot. The bots is useful when you don't require everyone in a channel to see what you are doing with a bot. For example, when having users respond to a poll in a channel and not useful to see a record of that poll being created. For more information, see [using task modules from Teams bots](~/task-modules-and-cards/task-modules/task-modules-bots.md).
...
## Components of a task module
 ...
 A task module looks like when invoked from a bot:

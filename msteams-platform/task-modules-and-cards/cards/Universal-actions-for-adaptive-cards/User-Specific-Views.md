...
* For User Specific Views, Teams client stores 50 latest cached cards per user. When a new card requires caching, the store removes the oldest card based on when it's last read and replaces it with the latest card. There's no process to restore the cards removed after a certain period. The user can also clear Teams cache to delete cards from the store.

...
The following image shows how SSO works when a Teams app user attempts to access Adaptive Cards Universal Actions in a bot:
...
* Adaptive Cards present in compose or preview area while the app user is composing the message. In the compose area, refresh in Adaptive Card works and the bot might want to use a token to provide a user-specific view to the app user before they send the card to the chat.

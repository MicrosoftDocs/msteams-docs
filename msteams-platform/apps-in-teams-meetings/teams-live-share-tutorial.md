...
Before you can begin receiving changes Live Share in the application, you must first call `initialize()` on your `LiveState` object with an initial value. This initial value will not overwrite any existing state that was sent by other users.

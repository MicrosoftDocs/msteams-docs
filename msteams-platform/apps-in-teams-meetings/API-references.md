...
1. To use the `shareAppContentToStage` API, you must obtain the RSC permissions. In the app manifest, configure the `authorization` property, and the `name` and `type` in the `resourceSpecific` field. For example:
    ...
    ```
2. `appContentUrl` must be allowed by `validDomains` array inside manifest.json, else API would return 501. 
### Query parameter

...
If you are hosting your bot locally during development, you'll need to use a tunneling service like [ngrok](https://ngrok.com/) to test your bot. Once you've downloaded and installed ngrok, run the below command to start the tunneling service. You may need to add ngrok to your path.
...
Users can delete the bot by choosing the trash-can icon on the bots list in their teams view. It only removes the bot from that team's use, individual users will still be able to interact in personal context.
...
To stop your bot receiving messages, go to your Bot Dashboard and edit the Microsoft Teams channel. Clear the **Enable on Microsoft Teams** option. It prevents users from interacting with the bot, but it will still be discoverable and users will still be able to add it to teams.

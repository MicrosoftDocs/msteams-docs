...
In general, the recommended approach is to sign the user in with the bot using the same identity system used to provide authentication for the tab content. You can implement via the action to sign in, which typically signs the user in via an OAuth flow.

...
* Bots can't proactively retrieve the `userPrincipalName` or `email` properties for members of a chat or team. Bots must use Graph APIs to retrieve the required information. The `GetConversationPagedMembers` API can't return the `userPrincipalName` and `email` properties.

...
With the introduction of organization wide teams, there's a requirement to better align these APIs with Microsoft 365 privacy controls. Bots used in large teams can retrieve basic profile information similar to the `User.ReadBasic.All` Microsoft Graph permission. Teams Administrators have a great deal of control over which apps and bots can be used in their tenant, but these settings are different from Microsoft Graph.
